...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	// Преобразуем скорость в метры в секунду
	speedInMetersPerSecond := speed * kmhInMsec

	// Расчёт калорий по формуле
	calories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(speedInMetersPerSecond, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
