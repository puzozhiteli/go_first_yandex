...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
    var distanceValue, speed, calories float64
    switch trainingType {
    case "Бег":
        distanceValue = distance(action)
    ...
    return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n",
        trainingType, duration, distanceValue, speed, calories)
}
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    return ((runningCaloriesMeanSpeedMultiplier * float64(action) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    speed := meanSpeed(action, duration) * kmhInMsec // переводим км/ч в м/с
    speedSquared := math.Pow(speed, 2)
    return ((walkingCaloriesWeightMultiplier * weight) + (speedSquared / height * walkingSpeedHeightMultiplier * weight)) * duration * minInH
}
